...
template<typename valType> ostream& foo(ostream& os, const BTnode<valType>& bt);

template<typename valType> class BTnode {
	friend class BinaryTree<valType>;
	//friend ostream& foo<valType>(ostream&, const BTnode<valType>&);
	//friend ostream& foo(ostream&, const BTnode<valType>&);
...
{
	if (val < _val) {
		if (!_lchild) return; //not present
		else _lchild->remove_value(val, _lchild);
	}
	else if (val > _val) {
		if (!_rchild) return; //not present
		...
		if (_rchild) {
			prev = _rchild;
			if (_lchild && !prev->_lchild) prev->_lchild = _lchild;//quate to put on rchild_lchild...leaf
...
find_value(const valType& val) const{
	...
	if (val < _val)
		if (!_lchild) return false;
		else return _lchild->find_value(val);
	else
		if (!_rchild) return false;
